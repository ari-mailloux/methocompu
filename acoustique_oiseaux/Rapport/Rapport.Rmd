...
print(donnees_heures)
objet<-subset(donnees_heures,heure_fo)
donn_non_na <- donnees_heure[!is.na(donnees_heure$heure_formattee), ]
print(donn_non_na)
```
...
```{r}
#dire ici de tar_read l'étape de création du graph2
```
...
```{r}
#dire ici de tar_read l'étape de création du graph3
```
